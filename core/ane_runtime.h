...
/// Completion callback for async evaluation.
typedef void (^OrionEvalCallback)(bool success);

/// Evaluate a compiled ANE program asynchronously.
/// The callback is invoked on the specified queue when evaluation completes.
/// Note: Currently wraps synchronous eval - the ANE framework doesn't have
/// native async evaluation. This API exists for future optimization.
/// @param prog        Compiled program handle
/// @param inputs      Array of input IOSurface tensors
/// @param num_inputs  Number of inputs
/// @param outputs     Array of output IOSurface tensors (pre-allocated)
/// @param num_outputs Number of outputs
/// @param queue       Dispatch queue for callback (NULL = main queue)
/// @param callback    Completion handler (may be NULL)
void orion_eval_async(
    OrionProgram* prog,
    IOSurfaceRef* inputs, int num_inputs,
    IOSurfaceRef* outputs, int num_outputs,
    dispatch_queue_t queue,
    OrionEvalCallback callback
);
