...
void orion_eval_async(
    ...
    // Retain inputs/outputs for async context
    // Note: IOSurfaceRef is already a shared reference, no need to retain

    dispatch_async(queue ?: dispatch_get_main_queue(), ^{
        bool ok = orion_eval(prog, inputs, num_inputs, outputs, num_outputs);
        if (callback) {
            callback(ok);
        }
    });
